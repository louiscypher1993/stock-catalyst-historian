...
**(1) `sector_relative_z_score` is genuinely unaddressed AND material — the headline of this stage.**
- **Code path confirmed** (three independent checks): (a) it is NOT in the `features` object assembled by `buildFeatureVectorForAnomaly()` (`LiveInferenceService.ts:627-667`), so `f.sector_relative_z_score` is always `undefined` in the live path; (b) `feature_extractor.ts:151` is `numOrNull(f.sector_relative_z_score ?? s?.sector_relative_z_score)` → always falls through to the snapshot value `s?.…`; (c) the Phenomenon 3 fresh-snap patch (`LiveInferenceService.ts:1203-1209`) strips only `z_score`/`excess_return`/`atr_shock_score`/`volume_ratio` from `freshSnap`, NOT this field. Net: live value is always the stale snapshot (frozen 2026-06-13) or, for zero-coverage symbols, null → 0 via `infer.py`'s `build_df` default. Training-side it IS computed (`HistoricalEngine.ts:2139-2160`, from `sectorReturnsMap`), so this is a true train/serve skew, not a dropped feature.
- **Materiality quantified** (gain-based importance, `xgboost` on the deployed v9.3 JSONs against `feature_metadata_v9.3.json`'s 72-col order): **D5 (2W) rank #2 = 10.47%**, D2 (6M) #3 = 5.15%, D1 (3M) #4 = 5.68%, B (1M) #16 = 0.31%, D3 (2D) #20 = 0.00%. **D5 is the canonical recommendation basis** (`getRecommendation` → `HORIZON_TIER_CONFIG.model_d5_return_2w`, `LiveInferenceService.ts:718/744/1245`) AND the 2W patience band; D1/D2 are the 3M/6M patience bands — i.e. the long-horizon bands the v10 sweep found most profitable (patience≈9).
- **Sensitivity confirmed** (200 real feature rows, D5, vary this one field): null→0 vs true `p75` (+3.66) flips the D5 tier in **10%** of rows; vs `p95` (+11.3), **20%** flip (max 2W-prediction swing 0.114). Training distribution: median exactly 0 but **74.5% of events have |z|>1** (std 81.5, p05/p95 = −9.29/+11.31), so pinning to 0 is unrepresentative for ~3/4 of events. Directional asymmetry: negative true values barely move D5 (splits live on the positive side); positive values do.
- **Disposition: flag for a decision, do NOT fix unilaterally.** A true live value needs the trailing-30-day sector-return *distribution* at scan time, which the one-symbol-at-a-time Yahoo scan path doesn't hold. Options (for Lewis to weigh): (i) compute a live sector-return proxy during the scan (needs the sector universe's daily returns threaded in — non-trivial); (ii) refresh `symbol_snapshots` via a re-run of `migrate_snapshots.ts` (only helps if local `event_features` is re-extracted first — expensive `HistoricalEngine` run — and even then gives "latest historical" not "live-at-scan"); (iii) accept the skew and document it as a known D5 limitation. No option is free; this is a genuine open decision, now quantified rather than hand-waved.
...
**Net for the plan:** (3) closed. (2) closed (subsumed by (1)). (1) escalated from "low-priority cosmetic gap" to a **quantified, material D5 train/serve skew awaiting a fix-vs-accept decision** — this is now the most consequential open item surfaced by the deep dive that isn't already scheduled.
