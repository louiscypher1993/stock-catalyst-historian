...
**DEPLOYED 2026-07-22 as v9.4** (sign-off given after a fresh read). The 5 validated candidate files were copied `scratch/model_*_v9.4cand.json` → `src/ml/model_*_v9.4.json` (byte-identical, verified), `feature_metadata_v9.4.json` deployed (72-col schema identical to v9.3, re-checked), and infer.py's **B/D1/D2/D3/D5 loads repointed to v9.4** (A/C/E stay v9.1, D4 stays v9.2). Verified end-to-end: infer.py loads all files and runs. **Live on the next scheduled cron** — confirmed `feature/local-development` is the repo's authoritative default branch (`git ls-remote --symref origin HEAD`), i.e. the branch GitHub's scheduled `live-inference` workflow runs off (the local `origin/HEAD`→main pointer was stale; `main` is an abandoned 2026-06-05 branch). **One-step rollback:** revert the 5 infer.py load lines to `_v9.3.json` — the v9.3 files are retained on disk untouched.
...
### Stage 5 — reporting/ops cluster (2026-07-22)

The v10 roadmap's remaining items (1/3/5; item 2 = the tracker, done; item 4 = IBKR, parked).

**Item 1 — top-3-buys-per-horizon report: BUILT.** `src/scripts/topBuysReport.ts` (standalone, read-only console; `npx tsx src/scripts/topBuysReport.ts [run_date]`). For each of the 5 horizons (2D/2W/1M/3M/6M → D3/D5/B/D1/D2) it ranks the latest `inference_results` run by that head's stored (clamped) prediction, shows the top 3, and classifies each via the **imported** `HORIZON_TIER_CONFIG` + `resolveTierFromConfig` (single source of truth — no threshold drift). `unreliable_reason`-flagged rows excluded (same set the outcome tracker drops). Marks clamp-ceiling hits as `(clamp)` and labels the 1M dead band explicitly. Verified against the 2026-07-22 run (72 scored, 27 excluded, 45 ranked; 4/15 slots clear a buy tier — all HOLD/SELL day otherwise). `tsc` clean.

**Item 3 — watchlist-pulse cadence + skip-if-just-scanned: already implemented, confirmed.** `watchlist-pulse.yml` runs a dependency-free price check (`scripts/watchlist-pulse.mjs`) *before* `npm ci` (no-op pulse <1 min), then only pays for full inference on triggered symbols. The skip gate (`watchlist-pulse.mjs:192-206`) is exactly the roadmap's "skip-if-just-scanned": re-triggers only on ≥1.5% move since last full score, or >4h stale + >0.05% move; `--mark-scored` promotes state only after a successful run. Cadence `*/15` Sun–Fri is sensible for a public repo. No change needed. Minor flag (not fixed, cosmetic): the `watchlist-pulse.yml:10-11` comment ("Cron ships commented out deliberately") is stale — the cron is live at line 21.

**Item 2 — live-scan-timing vs exchange hours: reviewed, well-aligned.** The 3×/day full-universe scan (`live-inference.yml`: 07:00 / 15:30 / 20:00 UTC, Mon–Fri) maps cleanly to the three major regional closes — Asia-Pacific (Tokyo/Korea/Taiwan/Sydney) at 07:00, Europe/UK + India at 15:30, Americas (US/Canada/Brazil) at 20:00. Residual gaps all minor: HK's ~08:00 close is first captured at 15:30 (same-day, ~7.5h delay); Middle-East Sunday sessions (`.SR`/Tadawul) fall outside the Mon–Fri full-universe scan but are covered for watched/held symbols by the Sun–Fri `*/15` pulse. Not a defect; no change.

**Stage 5 complete.** Remaining across the whole deep dive: the two parked heavy items only — v9.4cand deploy sign-off (read Stage 3 fresh) and the FMP-premium-gated re-extraction bundle (secStd floor + Phenomenon-1 universe expansion + v10 DROP).
