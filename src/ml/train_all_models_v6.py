...
MODEL_A_EXCLUDE_SNAP = True  # exclude all snap_* columns from Model A feature set

SNAPSHOT_LABEL_MIRRORS = [
    'snap_is_null_sample',
...
def main():
    ...
    if MODEL_A_EXCLUDE_SNAP:
        model_a_cols = [c for c in model_a_cols if not c.startswith('snap_')]
    ...
    model_a.save_model(str(ML_DIR / 'model_a_v6.json'))
    results.append({
        'model': 'A', 'file': 'model_a_v6.json', 'label': 'is_event',
        'train_rows': len(X_train), 'test_rows': len(X_test),
        'metric': 'AUC-ROC', 'score': f'{auc_a:.4f}',
    })
    print(f"\n[A] label=is_event train={len(X_train)} test={len(X_test)} "
          f"scale_pos_weight={scale_pos_weight:.4f} AUC={auc_a:.4f} -> model_a_v6.json")

    # --- Models B / C / D1-D5 ---
    train_regressor('B', 'model_b_v6.json', df, model_bcde_cols, 'forward_return_1m')
    train_regressor('C', 'model_c_v6.json', df, model_bcde_cols, 'max_adverse_excursion_1m')
    ...
    train_regressor('D1', 'model_d1_v6.json', df, model_bcde_cols, d1_label)

    train_regressor('D2', 'model_d2_v6.json', df, model_bcde_cols, 'forward_return_6m')
    train_regressor('D3', 'model_d3_v6.json', df, model_bcde_cols, 'forward_return_2d')
    train_regressor('D4', 'model_d4_v6.json', df, model_bcde_cols, 'forward_return_3d')
    train_regressor('D5', 'model_d5_v6.json', df, model_bcde_cols, 'forward_return_2w')
    ...
    model_e.save_model(str(ML_DIR / 'model_e_v6.json'))
    results.append({
        'model': 'E', 'file': 'model_e_v6.json', 'label': 'outperform_12m',
        'train_rows': len(X_train), 'test_rows': len(X_test),
        'metric': 'AUC-ROC', 'score': f'{auc_e:.4f}',
    })
    print(f"[E] label=outperform_12m train={len(X_train)} test={len(X_test)} AUC={auc_e:.4f} -> model_e_v6.json")
    ...
    with open(ML_DIR / 'feature_metadata_v6.json', 'w') as f:
        json.dump(metadata, f, indent=2)
    print("\nWrote feature_metadata_v6.json")
