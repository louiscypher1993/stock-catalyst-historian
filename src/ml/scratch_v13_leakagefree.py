...
# The SIX features measured 2026-08-13 to be constant within symbol while varying widely
# across symbols -- a present-day value stamped onto every historical row. gdelt_tone_z is
# also stamped (97.2%) but never reaches the model (already in ZERO_FILL_COLS), so it is
# not listed. stocktwits_virality_z is DELIBERATELY ABSENT: measured 1.5% constant within
# symbol, it is NOT stamped, and the old two-item list wrongly dropped it while keeping
# five genuine leaks -- which is why the v11 'nostamp' arm measured the wrong thing.
...
# Globally constant / all-zero columns that still reach the model. Dropping them must be
# EXACTLY neutral (XGBoost cannot split on a constant), so an arm that removes them is a
# validity check on this harness: if it moves the IC, the harness is not doing what it says.
DEAD = ['news_relevance_z', 'ctb_velocity_7d', 'iv_crush_pct', 'congressional_net_flow_30d',
        'av_news_sentiment', 'primaryCategory_technical', 'confidence_tier_low']
...
def main():
    ...
    feats_nodead = [c for c in feats_honest if c not in DEAD]
    print(f'features: v9.4 {len(feats_v94)}, leakage-free {len(feats_honest)}, '
          f'leakage-free+nodead {len(feats_nodead)}')
    ARMS = [
        ('v94-control', df, feats_v94),                        # inflated baseline, for reference
        ('honest-control', df, feats_honest),                  # THE NEW ANCHOR: all rows, 6 leaks gone
        ('honest-nodead', df, feats_nodead),                   # validity check: must equal honest-control
        ('honest-rowsonly', df[~pre21], feats_honest),         # row fix judged against honest baseline
        ('honest-rows-ne', df[~(pre21 | bad_ne)], feats_honest),
        ('honest-union', union, feats_honest),
    ]
    ...
    tag = Path(args.features_file).stem.replace('features_', '') if args.features_file else 'v13_leakagefree'
    ...
    if FOLD_MODE:
        ...
        print(f'\nwrote {OUT_DIR / "v13_leakagefree_folds.csv"}')
        return
    ...
    print(f'\nwrote {OUT_DIR / "v13_leakagefree_results.csv"}')
