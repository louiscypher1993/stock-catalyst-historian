...
def main():
    ...
    with open(ML_DIR / 'calibrator_a_v9.2.pkl', 'wb') as f:
    ...
    model_a.save_model(str(ML_DIR / 'model_a_v9.2.json'))
    results.append({
        'model': 'A', 'file': 'model_a_v9.2.json', 'label': 'is_event',
    ...
    print(f"\n[A] label=is_event train={len(X_train_a)} val={len(X_val_a)} test={len(X_test_a)} "
          f"scale_pos_weight={scale_pos_weight:.4f} AUC={auc_a:.4f} -> model_a_v9.2.json")
    print(f"[A] Brier score (raw):        {brier_raw:.4f}")
    print(f"[A] Brier score (calibrated): {brier_cal:.4f} -> calibrator_a_v9.2.pkl")

    plot_confusion_matrix_a(y_test_a, test_pred_a, 'confusion_matrix_a.png')

    # --- Models B / C / D1-D5 ---
    shap_store = {}
    train_regressor('B', 'model_b_v9.2.json', df, model_bcde_cols, 'forward_return_1m', shap_store)
    train_regressor('C', 'model_c_v9.2.json', df, model_bcde_cols, 'max_adverse_excursion_1m', shap_store)
    ...
    train_regressor('D1', 'model_d1_v9.2.json', df, model_bcde_cols, d1_label)

    train_regressor('D2', 'model_d2_v9.2.json', df, model_bcde_cols, 'forward_return_6m')
    train_regressor('D3', 'model_d3_v9.2.json', df, model_bcde_cols, 'forward_return_2d')
    train_regressor('D4', 'model_d4_v9.2.json', df, model_bcde_cols, 'forward_return_3d')
    train_regressor('D5', 'model_d5_v9.2.json', df, model_bcde_cols, 'forward_return_2w')
    ...
    model_e.save_model(str(ML_DIR / 'model_e_v9.2.json'))
    results.append({
        'model': 'E', 'file': 'model_e_v9.2.json', 'label': e_label,
    ...
    print(f"[E] label={e_label} train={len(X_train_e)} val={len(X_val_e)} test={len(X_test_e)} "
          f"AUC={auc_e:.4f} -> model_e_v9.2.json")
    ...
    with open(ML_DIR / 'feature_metadata_v9.2.json', 'w') as f:
        json.dump(metadata, f, indent=2)
    print("\nWrote feature_metadata_v9.2.json")
